...
public class FileService {
	...
	public void method1() {
		...
		// 제일 앞에 "/" 시작의 의미 : 현재 실행중인 프로젝트의 루트 디렉토리를 노리겠다.
		...
		File directory = new File("/io_test/20251014"); // 즉, C:가 생략되어있다는 것
		// File directory = new File("C:/io_test/20251014"); // 즉, 절대 경로로 작성된 것
		// 이렇게 생성해도 바로 만들어지지 않고, 연결을 한 상태
		
		// 폴더가 존재하지 않으면 생성
		
		if(!directory.exists()) {
			// 폴더를 모두 생성
			directory.mkdirs(); // 모두 생성 -> io_test와 20251014 폴더 생성
