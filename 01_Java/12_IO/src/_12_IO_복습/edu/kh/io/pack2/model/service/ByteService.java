...
public class ByteService {
	...
	public void fileByteOutPut() {
		...
		// 패키지가 java.io에서 가져온건데, IOException이 발생할 수 있기 때문에 try/catch문 안에 작성한다.
		try {
	...
	public void fileCopy() {
		...
		try {
			...
			// 출력용 스트림 생성
			fos = new FileOutputStream(sb.toString()); // StringBuilder 타입을 String 타입으로
			bos = new BufferedOutputStream(fos);
	...
	public void fileByteOutput1() {
		...
		FileOutputStream fos = null;
		// try/catch문 밖에서 객체를 생성하면 Unhandled exception type FileNotFoundException 발생 
		// -> 따라서 try 문 안에서 객체 생성
...
//			}
			...
			System.out.println("[수행시간] : " + (endTime - startTime) + "ns");
			fos.flush(); // FileOutputStream에서는 flush() 선택 사항, 하지만 쓴다고 생각
			// flush : 스트림안에 남아있는 모든 데이터를 외부로 모두 밀어냄
	 ...
	 */
	public void bufferedFileByteOutput1() {
		/*
		 * 버퍼(Buffer)란?
		 * - 데이터를 모아두는 공간, 마치 바구니 같은 메모리 공간
		 * 
		...
		try {
			...
			// close() 있을 때는 flush() 선택사항, 그래도 쓰는 것을 권장
	...
	public void fileByteInput1() {
		...
		try {
			...
			// FileInputStream 이용 시 해당 경로에 반드시
			// 실제 파일이 존재해야함!
			// -> FileNotFoundException: \io_test\20251014\노래가사1.txt (지정된 파일을 찾을 수 없습니다)
			...
			*/
			...
			try {
				// 사용완료된 Stream 메모리 반환
				if(fis != null) {
					fis.close(); // NullPointerException 방지
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
